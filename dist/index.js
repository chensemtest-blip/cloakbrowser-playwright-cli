...
function parseArgs(argv) {
    ...
    let proxy = process.env.CLOAKBROWSER_PROXY;
    const clean = [];
...
function findPlaywrightCliBin() {
    try {
        const pkgJsonPath = require.resolve("@playwright/cli/package.json");
        const pkgDir = path.dirname(pkgJsonPath);
        const pkg = require("@playwright/cli/package.json");
...
async function main() {
    ...
    if (stealth && command && SESSION_CMDS.has(command) && !hasConfig) {
        ...
        if (timezone)
            args.push(`--fingerprint-timezone=${timezone}`);
        cleanupOldConfigs();
